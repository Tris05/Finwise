...
import { useAgenticInsights } from "@/hooks/useAgenticInsights"
...
export default function DashboardPage() {
  ...
  const { score, recommendations, loading: insightsLoading } = useAgenticInsights()
  ...
  const kpis = [
    {
      ...
      delta: `${totalGainPercent >= 0 ? '+' : ''}${totalGainPercent.toFixed(1)}%`,
      trend: totalGainPercent >= 0 ? "up" : "down",
      icon: DollarSign,
      color: "text-green-600",
      bgColor: "bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/20",
      change: `${totalGain >= 0 ? 'Profit' : 'Loss'}: ${formatINR(Math.abs(totalGain))}`,
      period: "All Time"
    },
    {
      label: "Today's Performance",
      value: formatINR(dayChange),
      delta: `${dayChangePercent >= 0 ? '+' : ''}${dayChangePercent.toFixed(1)}%`,
      trend: dayChangePercent >= 0 ? "up" : "down",
      icon: TrendingUp,
      color: "text-blue-600",
      bgColor: "bg-gradient-to-br from-blue-50 to-cyan-50 dark:from-blue-950/20 dark:to-cyan-950/20",
      change: dayChangePercent >= 0 ? "Gaining momentum" : "Market dip",
      period: "Today"
    },
    {
      label: "Top Asset",
      ...
      change: investments.length > 0 ? formatINR(investments[0].currentPrice) : "-",
      period: "Live"
    },
    {
      label: "Investment Score",
      value: (invLoading || insightsLoading) ? "Calculating..." : score > 0 ? `${score}/1000` : "Run Optimizer",
      delta: score > 800 ? "Excellent" : score > 600 ? "Good" : score > 0 ? "Needs Review" : "N/A",
      trend: "up",
      icon: Award,
      color: "text-orange-600",
      bgColor: "bg-gradient-to-br from-orange-50 to-amber-50 dark:from-orange-950/20 dark:to-amber-950/20",
      change: score > 0 ? "AI Analysis Ready" : "No strategy yet",
      period: "This month"
    },
  ]
  ...
  const alerts = [
    ...(recommendations.length > 0 ? [{
      type: "info",
      icon: Bot,
      text: `AI Suggestion: ${recommendations[0].name || recommendations[0].rationale}`,
      time: "Just now",
      amount: "New",
    ...
    ...(goals.filter(g => (g.currentAmount / g.targetAmount) > 0.8).map(goal => ({
      type: "success",
      icon: Target,
      text: `${goal.name} is ${Math.round((goal.currentAmount / goal.targetAmount) * 100)}% complete!`,
      time: "Goal Health",
      amount: formatINR(goal.currentAmount),
      action: "Track progress"
    }))),
    {
      type: "warning",
      icon: Bell,
      text: "Security check recommended - ensure 2FA is active",
      time: "System",
                ...
                <CardContent className="relative p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    >
                      <Badge
                        variant="secondary"
                        className={`bg-white/90 text-gray-700 shadow-sm ${kpi.trend === "up" ? "text-green-600" : "text-red-600"
                          }`}
                      >
